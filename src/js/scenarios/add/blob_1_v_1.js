(function () {
  ...
  scenario.prepare = function () {
    ...
    for (var i = 0; i < dim; i++) {
      for (var j = 0; j < dim; j++) {
        blob.push(height * Math.exp(-(Math.pow(i - x,2) + Math.pow(j - x, 2)) / dimSigma2));
      }
    }
    this.blob = blob;
  };
   ...
   */
  scenario.add = function(inds, blob){
    ...
    tcoef=[1,this.mainSize];bcoef=[1,this.blobSize];
    hei=inds[2];
