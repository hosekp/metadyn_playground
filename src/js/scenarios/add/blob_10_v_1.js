(function () {
  var scenario = new metadyn.Scenario("Add blob 10:1", 'Add');
  metadyn.AddScenario(scenario);
  var RATIO = 10;
  scenario.prepare = function () {
    this.data = this.prepareDataArray(this.mainSize);
    this.blob = this.populateBlob([], this.blobSize*RATIO);
  };
  scenario.syncScenario = function syncTest() {
    ...
    this.blobAdd([x,y,height],{data:blob,dims:[this.blobSize*RATIO,this.blobSize*RATIO]});
  };
  /**
   *
   * @param {Array.<number>} inds
   * @param {{data:Array<number>,dims:Array.<number>}} blob
  ...
  scenario.blobAdd = function(inds, blob){
    ...
    for(i=0;i<2;i+=1){
      ...
      gtmid=Math.round(inds[i]*blob.dims[i]); // this.mid in greater resolution
    ...
    tcoef=[1,this.mainSize];bcoef=[1,blob.dims[0]];
    hei=inds[2];
